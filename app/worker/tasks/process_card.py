    ...
    try:
        ...
        if card_path.lower().endswith(".xls"):
            ...
            temp_xls_path = os.path.join(
                job_dir, f"temp_{job_id}_{os.path.basename(card_path)}"
            )
        ...
        # Write to temporary file to open with openpyxl and analyze sheets
        temp_source_path = os.path.join(job_dir, f"split_src_{job_id}_{filename}")
        os.makedirs(job_dir, exist_ok=True)
        ...
        if split_files:
            ...
            for sf in split_files:
                ...
                for p in sf_parse_result.parts:
                    # Double-check strike filter to exclude canceled parts
                    sf_parts.append(
                        {
                            "card_no": sf_card_no,
